...
def make_guess(n):
    ...
    >>> a
    4
    """#guesser is a new guess function
    def update_guess(num_incorrect):#num_incorrrect is a parameter that should be chanded by programmer, so each function being evaluated is a 'newguess' function
        def new_guess(x):
            if x==n:
        ...
        return new_guess
    return update_guess(0)#if use "num_incorrect+=1,the num_incorrect is a current number, it isn't a nonlocal varible"
