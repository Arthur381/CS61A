...
def is_arm(s):
    ...
    return type(s) == list and len(s) == 3 and s[0] == 'arm'#three srandards

def length(s):#read the length of the arm
...
def total_weight(m):#demonstrate
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        a=left(m)
        b=right(m)
        return balanced(end(a)) and balanced(end(b)) and total_weight(end(a))*length(a)==total_weight(end(b))*length(b)
...
def replace_leaf(t, find_value, replace_value):#read a tree, and you must return a tree
...
def preorder(t):#more [ ] than needed
