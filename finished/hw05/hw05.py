...
def make_withdraw(balance, password):
    ...
    True
    """
    "*** YOUR CODE HERE ***"#even if the right code
    lst=[]
    def check(amount,word):
        nonlocal lst
        nonlocal balance
        if len(lst)<3 and word==password:
            if amount<=balance:
                balance=balance-amount
                return balance
            else:
...
def merge(incr_a, incr_b):#how to return a generator,use yield
...
def remainders_generator(m):#生成器里面有一个生成器
    """
    ...
    """
    "*** YOUR CODE HERE ***"
    def rem(n):#rem is generator
        for nature in naturals():
            yield nature*m+n
    yield rem(0)
